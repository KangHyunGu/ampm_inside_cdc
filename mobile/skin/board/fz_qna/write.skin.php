...
?>
	...
	<section id="m_sub1">
		...
		<div class="visual_banner">
			<img src="<?=G5_URL?>/images/sub_banner4.jpg" alt="질문답변 배너">
		</div>
		...
		<section id="bo_w">
					...
					<table>
						<tbody>
							...
							<tr>
								<th>답변요청</th>
								<td>
									<p class="opt">답변을 요청할 지정 마케터가 있으신가요?</p>
									<div class="radio_marketer">
										<span><input type="radio" name="mkt_chk" value="no" checked>전체</span>
										<span><input type="radio" name="mkt_chk" value="yes">선택 <span id="mkt_sch" style="cursor:pointer;">조회</span></span>
									</div>
									<div class="opt-s">*전체 마케터를 대상으로 질문을 등록합니다.</div>
								</td>
							</tr>
							<script>

								$(document).ready(function(){
								//지정마케터가 있다면 답변요청 초기값을 선택으로 처리
								<?php
								if(($w=='u' && $write['wr_11']) || (isset($mk_id) && $mk_id != '')){
								?>
									var mkt_chk = 'yes';
									$('#mkt_sch').css('visibility','visible');
									$('.opt-s').css('display','none');
									
									$("#mk_appoint").css('visibility','visible');
								<?php
									if((isset($mk_id) && $mk_id != '')){
								?>
								...
								?>
									$("input:radio[name='mkt_chk']").val([mkt_chk]);

									// 라디오버튼 클릭시 이벤트 발생
									$("input:radio[name=mkt_chk]").click(function(){
										var valueCheck = $('input:radio[name=mkt_chk]:checked').val(); 
										
										if ( valueCheck == 'no' ) {
											$('#mkt_sch').css('visibility','hidden');
											$('.opt-s').css('display','block');
										} else {
											$('#mkt_sch').css('visibility','visible');
											$('.opt-s').css('display','none');
										}
			...
			<?php include_once(G5_PATH.'/inc/_inc_qna_modal.php');?>
