...
//마이페이지용.- feeris
function get_pretty_mypage_url($folder, $no='', $query_string='', $action='', $go_table='')
{
    global $g5, $config;
    ...
    if( $url = run_replace('get_pretty_mypage_url', $url, $folder, $no, $query_string, $action) ){
        return $url;
    }
    ...
    if($config['cf_bbs_rewrite']) {
        ...
        if(in_array($folder, $boards)) {
            $url = G5_BBS_URL. '/mypage.php?bo_table='. $folder.'&go_table='.$go_table;
            if($no) {
                $url .= '&amp;wr_id='. $no;
            }
            if($query_string) {
...
// 짧은 주소 형식으로 만들어서 가져온다.
//관리자모드용.- feeris
function get_pretty_adm_url($folder, $no='', $query_string='', $action='')
{
    global $g5, $config;
    ...
    if( $url = run_replace('get_pretty_adm_url', $url, $folder, $no, $query_string, $action) ){
        return $url;
    }
    ...
    if($config['cf_bbs_rewrite']) {
        ...
        if(in_array($folder, $boards)) {
            $url = G5_ADMBBS_URL. '/board.php?bo_table='. $folder;
            if($no) {
                $url .= '&amp;wr_id='. $no;
            }
            if($query_string) {
