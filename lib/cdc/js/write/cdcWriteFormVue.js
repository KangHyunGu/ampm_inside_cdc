const app = Vue.createApp({
    ...
    methods: {
        ...
        async setImages() {
            ...
            const videoFiles = this.bf_file.filter((item) => {
                return item != null && item.type == 'video/mp4';
            })

            //비디오 타입 처음 로드시 이곳에 처리
            const targetRefKeys = ['inputvideo', 'inputvideoYoutube'];

            for (let i = 0; i < targetRefKeys.length; i++) {
                if (videoFiles[i]) {
                    this.$refs[targetRefKeys[i]].uploaderFile = videoFiles[i];
                }
            }
        },
        ...
        async formSave($event) {
            ...
            //유튜브 태그
            //formData.set('wr_ytag', this.form.cdc.wr_ytag);
            appendData['wr_ytag'] = this.form.cdc.wr_ytag;
            appendData['wr_mhash'] = this.form.cdc.wr_mhash;
            appendData['wr_shash'] = this.form.cdc.wr_shash;
            ...
            for (const appendKey of appendKeys) {
                ...
                console.log('val : ', appendData[appendKey]);
                console.log('typeOf : ', typeof (appendData[appendKey]));
                if (typeof (appendData[appendKey]) == 'object') {
                    //이미지 및 첨부파일 삭제
                    for (const file of appendData[appendKey]) {
                        console.log('file : ', file);
                        const input = this.createInput(appendKey, file, 'file');
