const app = Vue.createApp({
    ...
    computed: {
		...
		*/
		//인사이트 게시판이고 블로그나 인스타가 체크가 된 경우
        isImageForm() {
            return this.bo_table == 'insight' && (this.form.cdc.is_blog == 'Y' || this.form.cdc.is_insta == 'Y')
        },
        ...
        // 해시태그
        // 2022.12.05 수정 개선사항 요청
        // 메인 해시태그, 서브 해시태그 통합(최대 30개)
        isHashTags() {
            return this.form.cdc.is_blog == 'Y' ||
                this.form.cdc.is_insta == 'Y' ||
    ...
    watch: {
        ...
        hashs() {
            this.form.cdc.wr_hash = Object.values(this.hashs).join('|');
        },
    ...
    methods: {
        fetchData(write_fields) {
            if (write_fields.wr_id) {
                ...
                if (!this.form.cdc) {
                    ...
                    // 해시태그, 유튜브 태그
                    const hashTagFields = {'wr_hash' : 'hashs', 'wr_ytag':'ytags'};
        ...
        async formSave($event) {
            ...
            // 유튜브 태그, 해시태그
            for (const tagFields of ['wr_ytag', 'wr_hash']) {
        ...
        autoSave() {
            ...
            const saveCdcTargets = ['is_blog', 'is_insta', 'is_youtube',
                'wr_hash', 'wr_video_link', 'wr_youtube_link',
        ...
        addHashTags(val, done){
            // 해시태그 최대 입력 30개
            // 2022.12.05 개선사항 요청
            if(this.hashs.length >= 30) return;
            done(val, 'add-unique');
        },
