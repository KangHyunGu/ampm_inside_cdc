    ...
    methods: {
        ...
        cdcModalOn(index) {
            ...
            if (isOpen) {
                ...
                //해시태그
                let hTags = this.curList.wr_hash.split("|")
        ...
        comp_link_action(type) {
            //작성한 블로그, 인스타, 유튜브 이동

            if (this.curList['is_' + type] == 'N' || !this.curList['wr_comp_' + type + '_link']) return;
            clickToAction(this.curList['wr_comp_' + type + '_link']);
        },
        ...
        async copy(fieldName, title) {
            let copyStr = '';
            if (fieldName == 'wr_cdc_content') {
                const content = this.curList[fieldName];
        ...
        async allDownload() {
            for (const imgInfo of this.curImgInfos) {
                await setTimeout(() => {
                    const downloadUrl = imgInfo.imgSrc;
                    const fileName = imgInfo.source;
                    this.imgDownload(downloadUrl, fileName);
                }, 700);
            }
        },
...
app.component('cdc_comp_form', compForm);
app.use(Quasar, {
    config: {
