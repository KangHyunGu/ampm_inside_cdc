...
function get_paging($write_pages, $cur_page, $total_page, $url, $add="")
{
    ...
    if ($start_page > 1) $str .= '<a href="'.$url.($start_page-1).$add.'" class="pg_page pg_prev"><i class="fas fa-angle-left"></i></a>'.PHP_EOL;
    ...
    if ($total_page > $end_page) $str .= '<a href="'.$url.($end_page+1).$add.'" class="pg_page pg_next"><i class="fas fa-angle-right"></i></a>'.PHP_EOL;
...
// 경고메세지 출력후 페이지 이동
function alert_move($msg, $url='', $error=true)
{
    global $g5, $config, $member, $is_member, $is_admin, $board;

    run_event('alert_move', $msg, $error);
...
function is_admin($mb_id)
{
    ...
    if ($config['cf_admin'] == $mb_id){
		...
		//////////////////////////////////////////////////////////////////
		//인트라넷 사원 중 팀장 이상 관리자 추가
		//메뉴접근권한 부여 하고 상하직하 리스트 조회 권한 부여
		//////////////////////////////////////////////////////////////////
		//운영관리자 계정
		if ($mb_id == 'manager') $is_authority =  'manager';

		//CDC 업로드관리자 계정
		if ($mb_id == 'cdc') $is_authority =  'uploader';

		$mb = get_marketer($mb_id);

		if($mb && $mb['mb_level'] == 2){	//퇴사자 제외, 관리자 제외
			if($mb['mb_post'] == 'P1'){
				$is_authority = 'mk_admin';
			}else{
				$is_authority = 'ma_admin';
			}
		}
	}
	//echo $is_authority;
...
function member_delete($mb_id)
{
    ...
    $sql = " update {$g5['member_table']} set mb_password = '', mb_level = 1, mb_email = '', mb_homepage = '', mb_tel = '', mb_hp = '', mb_zip1 = '', mb_zip2 = '', mb_addr1 = '', mb_addr2 = '', mb_birth = '', mb_sex = '', mb_signature = '', mb_leave_date = '".date('Ymd', G5_SERVER_TIME)."', mb_memo = '".date('Ymd', G5_SERVER_TIME)." 삭제함\n".sql_real_escape_string($mb['mb_memo'])."' where mb_id = '{$mb_id}' ";
...
// 게시물 정보($write_row)를 출력하기 위하여 $list로 가공된 정보를 복사 및 가공
function get_admlist($write_row, $board, $skin_url, $subject_len=40)
{
    global $g5, $config;
    global $qstr, $page;
    ...
    // 분류명 링크
    $list['ca_name_href'] = G5_ADMBBS_URL.'/board.php?bo_table='.$board['bo_table'].'&amp;sca='.urlencode($list['ca_name']);

    $list['href'] = G5_ADMBBS_URL.'/board.php?bo_table='.$board['bo_table'].'&amp;wr_id='.$list['wr_id'].'&amp;'.$qstr;
    $list['comment_href'] = $list['href'];
    ...
    for ($i=1; $i<=G5_LINK_COUNT; $i++) {
        $list['link'][$i] = set_http(get_text($list["wr_link{$i}"]));
        $list['link_href'][$i] = G5_ADMBBS_URL.'/link.php?bo_table='.$board['bo_table'].'&amp;wr_id='.$list['wr_id'].'&amp;no='.$i.$qstr;
        $list['link_hit'][$i] = (int)$list["wr_link{$i}_hit"];
    }
...
// 게시물 정보($write_row)를 출력하기 위하여 $list로 가공된 정보를 복사 및 가공
function get_marketerAdmlist($write_row, $board, $skin_url, $subject_len=40)
{
    global $g5, $config;
    global $qstr, $page;
    ...
    // 분류명 링크
    $list['ca_name_href'] = G5_MARKETER_ADMBBS_URL.'/board.php?bo_table='.$board['bo_table'].'&amp;sca='.urlencode($list['ca_name']);

    $list['href'] = G5_MARKETER_ADMBBS_URL.'/board.php?bo_table='.$board['bo_table'].'&amp;wr_id='.$list['wr_id'].'&amp;'.$qstr;
    $list['comment_href'] = $list['href'];
    ...
    for ($i=1; $i<=G5_LINK_COUNT; $i++) {
        $list['link'][$i] = set_http(get_text($list["wr_link{$i}"]));
        $list['link_href'][$i] = G5_MARKETER_ADMBBS_URL.'/link.php?bo_table='.$board['bo_table'].'&amp;wr_id='.$list['wr_id'].'&amp;no='.$i.'&amp;'.$qstr;
        $list['link_hit'][$i] = (int)$list["wr_link{$i}_hit"];
    }
...
// 게시물 정보($write_row)를 출력하기 위하여 $list로 가공된 정보를 복사 및 가공
function get_lastMarketerlist($write_row, $board, $skin_url, $subject_len=40, $sublink="")
{
    global $g5, $config;
    global $qstr, $page;
    ...
    $list['href'] = G5_BBS_URL.'/board.php?bo_table='.$board['bo_table'].'&amp;wr_id='.$list['wr_id'].'&amp;'.$qstr.$sublink;
    $list['comment_href'] = $list['href'];
...
// get_list 의 alias
function get_admview($write_row, $board, $skin_url)
{
    return get_admlist($write_row, $board, $skin_url, 255);
}

// get_list 의 alias
function get_marketerAdmview($write_row, $board, $skin_url)
{
    return get_marketerAdmlist($write_row, $board, $skin_url, 255);
}

// get_list 의 alias
function get_lastMarketerView($write_row, $board, $skin_url)
{
    return get_lastMarketerlist($write_row, $board, $skin_url, 255);
}
...
//마케터 개인화 홈페이지 관련
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
function sql_query_intra($sql, $error=G5_DISPLAY_SQL_ERROR, $link=null)
{
    global $g5, $g5_debug;

    if(!$link)
        $link = $g5['connect_intra_db'];
...
// 쿼리를 실행한 후 결과값에서 한행을 얻는다.
function sql_fetch_intra($sql, $error=G5_DISPLAY_SQL_ERROR, $link=null)
{
    global $g5;
    ...
    $result = sql_query_intra($sql, $error, $link);
    //$row = @sql_fetch_array($result) or die("<p>$sql<p>" . mysqli_errno() . " : " .  mysqli_error() . "<p>error file : $_SERVER['SCRIPT_NAME']");
    $row = sql_fetch_array($result);
    return $row;
}
...
// 인트라넷 사원 정보를 얻는다.
function get_marketer($mb_id, $fields='*')
{
    global $g5, $bdd;

	//$sql = " select $fields from {$g5['member_table']} where mb_id = TRIM('$mb_id') AND mb_level > 1 ";
	$sql = " select $fields from {$g5['member_table']} where mb_id = TRIM('$mb_id') ";
	//echo $sql; exit;
	return sql_fetch_intra($sql);
	/*
	$req = $bdd->prepare($sql);
	$req->execute();
	$fetch = $req->fetch();
