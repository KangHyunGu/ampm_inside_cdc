...
function valid_mb_id($reg_mb_id){	//feeris - 아이디 이메일 사용에 따른 변경
	/*
...
function exist_mb_id($reg_mb_id)
{
    ...
    if ($row['cnt']){
		...
		//인사이드-메일
		$sql = " select count(*) as cnt from `{$g5['member_table']}` where mb_email = '$reg_mb_id' ";
		$row = sql_fetch($sql);
		if ($row['cnt']){
			return "이미 사용중인 회원아이디 입니다.";
		}else{
...
function exist_mb_nick($reg_mb_nick, $reg_mb_id)
{
    ...
    if ($row['cnt']){
		...
		//인트라넷
		$row = sql_fetch_intra(" select count(*) as cnt from {$g5['member_table']} where mb_nick = '$reg_mb_nick' and mb_id <> '$reg_mb_id' ");
		if ($row['cnt']){
			return "이미 존재하는 닉네임입니다.";
		}else{
	        return "";
