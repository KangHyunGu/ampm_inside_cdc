...
<div class="tbl_head01 tbl_wrap">
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
		...
		// 본DB에 회원아이디가 없는 메뉴는 삭제함
        if($row['mb_id'] == '' && $row['mb_nick'] == '') {

			//메뉴권한 테이블의 정보 추출
			$au = get_menu_auth($row['au_menu']);
			
			//인트라넷 서버에 메뉴권한 테이블의 mb_id 정보가 있는지 체크
			$ck = get_marketer($au['mb_id']);
			...
			if(!$ck['mb_id']){
				...
				//마케터인사이DB의 메뉴권한테이블의 회원정보가 아닌경우 
				//인트라넷 서버의 사원정보로 치환

				$row['mb_id']		= $ck['mb_id'];
				$row['mb_nick']		= $ck['mb_nick'];
				$row['mb_name']		= $ck['mb_name'];
				$row['mb_email']	= $ck['mb_email'];
				$row['mb_homepage'] = $ck['mb_homepage'];
			}
        }
...
<section id="add_admin">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            <th scope="row">회원DB 구분</th>
            <td>
				<select name="au_auth_ck" id="au_auth_ck" required class="required">
					<option value=''> 회원DB 구분 </option>
					<?=codeToHtml($code_authck, "", "cbo", "")?>
				</select>
            </td>
        </tr>
        <tr>
