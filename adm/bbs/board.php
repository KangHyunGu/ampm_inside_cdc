...
if (isset($write['wr_is_comment']) && $write['wr_is_comment']) {
    goto_url(get_pretty_adm_url($bo_table, $write['wr_parent'], '#c_'.$wr_id));
}
...
if ((isset($wr_id) && $wr_id) || (isset($wr_seo_title) && $wr_seo_title)) {
    ...
    if (!$write['wr_id']) {
        $msg = '글이 존재하지 않습니다.\\n\\n글이 삭제되었거나 이동된 경우입니다.';
        alert($msg, get_pretty_adm_url($bo_table));
    }
    ...
    if (isset($group['gr_use_access']) && $group['gr_use_access']) {
        if ($is_guest) {
            ...
            alert($msg, G5_BBS_URL.'/login.php?wr_id='.$wr_id.$qstr.'&amp;url='.urlencode(get_pretty_adm_url($bo_table, $wr_id, $qstr)));
        }
    ...
    if ($member['mb_level'] < $board['bo_read_level']) {
        if ($is_member)
            ...
            alert('글을 읽을 권한이 없습니다.\\n\\n회원이시라면 로그인 후 이용해 보십시오.', G5_BBS_URL.'/login.php?wr_id='.$wr_id.$qstr.'&amp;url='.urlencode(get_pretty_adm_url($bo_table, $wr_id, $qstr)));
    }
...
include_once(G5_ADMBBS_PATH.'/board_head.php');

// 게시물 아이디가 있다면 게시물 보기를 INCLUDE
if (isset($wr_id) && $wr_id) {
    include_once(G5_ADMBBS_PATH.'/view.php');
}

// 전체목록보이기 사용이 "예" 또는 wr_id 값이 없다면 목록을 보임
//if ($board['bo_use_list_view'] || empty($wr_id))
if ($member['mb_level'] >= $board['bo_list_level'] && $board['bo_use_list_view'] || empty($wr_id))
    include_once (G5_ADMBBS_PATH.'/list.php');

include_once(G5_ADMBBS_PATH.'/board_tail.php');
