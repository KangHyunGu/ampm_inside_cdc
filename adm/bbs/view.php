...
if ($sca || $stx || $stx === '0') {
    ...
    $search_href = './board.php?bo_table='.$bo_table.'&amp;page='.$page.$qstr;		// 검색 후 리턴 페이지
    $list_href = './board.php?bo_table='.$bo_table;									// 목록 페이지
} else {
    $search_href = '';
...
if (($member['mb_id'] && ($member['mb_id'] === $write['mb_id'])) || $is_admin) {
    ...
    $update_href = G5_ADMBBS_URL.'/password.php?w=u&amp;bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;page='.$page.$qstr;
    $delete_href = G5_ADMBBS_URL.'/password.php?w=d&amp;bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;page='.$page.$qstr;
}

// 최고, 그룹관리자라면 글 복사, 이동 가능
$copy_href = $move_href = '';
if ($write['wr_reply'] == '' && ($is_admin == 'super' || $is_admin == 'group')) {
    $copy_href = G5_ADMBBS_URL.'/move.php?sw=copy&amp;bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;page='.$page.$qstr;
    $move_href = G5_ADMBBS_URL.'/move.php?sw=move&amp;bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;page='.$page.$qstr;
}
...
if ($is_member) {
    // 스크랩 링크
    $scrap_href = G5_ADMBBS_URL.'/scrap_popin.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id;

    // 추천 링크
    if ($board['bo_use_good'])
        $good_href = G5_ADMBBS_URL.'/good.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;good=good';

    // 비추천 링크
    if ($board['bo_use_nogood'])
        $nogood_href = G5_ADMBBS_URL.'/good.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;good=nogood';
}

$view = get_admview($write, $board, $board_skin_path);
