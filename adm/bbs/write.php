...
if ($w == '') {
    if ($wr_id) {
        alert('글쓰기에는 \$wr_id 값을 사용하지 않습니다.', G5_ADMBBS_URL.'/board.php?bo_table='.$bo_table);
    }
...
if (!empty($group['gr_use_access'])) {
    ...
    if ($is_admin == 'super' || $group['gr_admin'] === $member['mb_id'] || $board['bo_admin'] === $member['mb_id'] || $is_admin == 'master') {
        ; // 통과
    } else {
        // 그룹접근
...
$action_url = https_url(G5_ADMBBS_DIR)."/write_update.php";
