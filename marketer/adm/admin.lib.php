...
function icon($act, $link='', $target='_parent')
{
    ...
    $icon = '<img src="'.G5_MARKETER_ADMIN_PATH.'/img/icon_'.$img[$act].'.gif" title="'.$act.'">';
    if ($link)
...
// 접근 권한 검사
if (!$member['mb_id']){
    alert('로그인 하십시오.', G5_MARKETER_BBS_URL.'/login.php?url=' . urlencode(G5_MARKETER_ADMIN_URL));
//////////////////////////////////////////////////////////////////
//마케터 사원 접근 권한 - feeris
//////////////////////////////////////////////////////////////////
}else if ($is_admin != 'super' && $is_admin != 'manager' && $is_admin != 'uploader' && $is_admin != 'ma_admin' && $is_admin != 'mk_admin'){
    $auth = array();
...
unset($menu);
unset($amenu);
$tmp = dir(G5_MARKETER_ADMIN_PATH);
...
while ($entry = $tmp->read()) {
    ...
    $amenu[$m[1]] = $entry;
    include_once(G5_MARKETER_ADMIN_PATH.'/'.$entry);
}
@ksort($amenu);
