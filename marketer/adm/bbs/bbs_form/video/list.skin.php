...
<div id="bo_gall" style="width:<?php echo $width; ?>">
    ...
    <div class="bo_fx">
        ...
        <ul class="btn_bo_user">
            ...
            <?php $admin_href=false;if ($admin_href) { ?><li><a href="<?php echo $admin_href ?>" class="btn_admin">관리자</a></li><?php } ?>
    ...
    <?php } ?>
	<?php echo help('[노출], [비노출] 표기는 공통 자료일 경우 표기되며 공통자료는 개인화페이지에 노출여부를 상세보기 화면에서 변경이 가능합니다.') ?>
    <ul id="gall_ul">
            ...
            <ul class="gall_con2">  <!-- 제목,내용 출력 -->
                <li class="gall_text_href">
                    ...
                    if ($list[$i]['is_notice']) { // 공지사항  ?>
                        <strong style="width:<?php echo $thumb_width ?>px;height:45px">[대표영상]</strong><br>
                    <?php } ?>
                ...
                <li>
                    ...
                    <?php } ?>
					<?php echo $list[$i]['wr_9'] ?> <span class="bo_gubun">|</span>        <!-- 작성자 -->
                    <?php echo date("Y-m-d", strtotime($list[$i]['wr_datetime'])) ?> <span class="bo_gubun">|</span>   <!-- 작성일 -->
				...
				<?php 
					...
					if ($list[$i]['mb_id'] == 'ampm') {
						$visible_array = explode(',', trim($list[$i]['wr_20']));

						$is_visible = false;
						$visible_checked = 'Y';
						$visible_check = $member['mb_id'].'|'.$list[$i]['wr_id'];
						if (in_array($visible_check, $visible_array)) {
							$visible_checked = 'N';

							echo '<li class="bo_content"><strong>[비노출]</strong></li>';
						}else{
							echo '<li class="bo_content"><strong>[노출]</strong></li>';
						}
...
<fieldset id="bo_sch">
    ...
    <input type="submit" value="검색" class="btn_design2 btn_submit">
    </form>
</fieldset>
<!-- } 게시물 검색 끝 -->
