...
if (file_exists($dbconfig_file)) {
	...
	// 인트라넷 데이터베이스 컨넥션
	$connect_intra_db = sql_connect(G5_MYSQL_HOST2, G5_MYSQL_USER2, G5_MYSQL_PASSWORD2, G5_MYSQL_DB2) or die('MySQL Connect Error!!!');
	$select_intra_db  = sql_select_db(G5_MYSQL_DB2, $connect_intra_db) or die('MySQL DB Error!!!');

	// mysql connect resource $g5 배열에 저장 - 명랑폐인님 제안
	$g5['connect_intra_db'] = $connect_intra_db;
...
if (isset($_SESSION['ss_mb_id']) && $_SESSION['ss_mb_id']) { // 로그인중이라면
	///////////////////////////////////////////////////////////////////
	//AMPM 사원 로그인 시 세션생성 추가 - feeris
	///////////////////////////////////////////////////////////////////
	if($_SESSION['ss_mb_ampm'] == 'Y' || $_SESSION['ss_mb_marketer'] == 'Y'){
		$member = get_marketer($_SESSION['ss_mb_id']);
	}else{
...
if (G5_IS_MOBILE) {
    ...
    $mypage_skin_path   = get_skin_path('mypage', $config['cf_1_subj']);//통합게시판 추가
    $mypage_skin_url    = get_skin_url('mypage', $config['cf_1_subj']);
    ...
    $mypage_skin_path   = get_skin_path('mypage', $config['cf_1']);
    $mypage_skin_url    = get_skin_url('mypage', $config['cf_1']);
...
//AMPM 사원 로그인 시 세션생성 추가 - feeris
///////////////////////////////////////////////////////////////////
$member['mb_part_text'] = ($member['mb_part'])?codeToName($code_part3, get_text($member['mb_part']))." 본부":"";
