<?php
	...
	*           본 예제에서는 보내는 사람의 핸드폰번호, 받는사람 핸드폰번호,전송내용 lms_main.php에서
	*           전송 받아 실제 처리하는  Lms 모듈입니다.
	*/
...
class LMS
{
	...
	function LMS($_webService)
	{
		$this->webService = $_webService;
	}

	function SendLMS($lms_id,$lms_pwd,$snd_number,$rcv_number,$lms_title,$lms_content)
	{
		$snd_number=str_replace("-","",$snd_number);
		$rcv_number=str_replace("-","",$rcv_number);
		$hash_value=md5($lms_id.$lms_pwd.$rcv_number);//해쉬값을 생성한다.
		...
		if(preg_match("'^[0-9,]+$'",$snd_number.$rcv_number)){
			...
			$result = $soapclient->call('SendLMS',$parameters); //실제적으로 전송을 실시하고 결과 값을 받습니다.
			return $result['SendLMSResult'];
		}
		else {
			return "-50";
		}
	...
	function SendLMSReserve($lms_id,$lms_pwd,$snd_number,$rcv_number,$lms_title,$lms_content,$reserve_date,$reserve_time,$userdefine)
	{
		$snd_number=str_replace("-","",$snd_number);
		$rcv_number=str_replace("-","",$rcv_number);
		$hash_value=md5($lms_id.$lms_pwd.$rcv_number);//해쉬값을 생성한다.
		...
		if(preg_match("'^[0-9,]+$'",$snd_number.$rcv_number)){
			...
			$result = $soapclient->call('SendLMSReserve',$parameters); //실제적으로 전송을 실시하고 결과 값을 받습니다.
			return $result['SendLMSReserveResult'];
		}
		else {
			return "-50";
		}
	...
	function GetRemainCount($lms_id,$lms_pwd)
		{
			$hash_value=md5($lms_id.$lms_pwd);
			$soapclient = new nusoap_client($this->webService,true);
			$soapclient->xml_encoding = "UTF-8";
			$soapclient->soap_defencoding = "UTF-8";
			$soapclient->decode_utf8 = false;
	...
	function ReserveCancle($lms_id,$lms_pwd,$userdefine)
		{
			$hash_value=md5($lms_id.$lms_pwd.$userdefine);
			$soapclient = new nusoap_client($this->webService,true);
			$soapclient->xml_encoding = "UTF-8";
			$soapclient->soap_defencoding = "UTF-8";
			$soapclient->decode_utf8 = false;
