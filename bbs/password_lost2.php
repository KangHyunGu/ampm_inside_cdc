...
$subject = "[".$config['cf_title']."] 임시 비밀번호가 발급되었습니다.";

$content = "";
include_once('./password_update_mail.php');

$content = preg_replace("/{회원이름}/", addslashes($mb['mb_name'])." (".addslashes($mb['mb_nick']).")", $content);
$content = preg_replace("/{회원아이디}/", $mb['mb_id'], $content);
$content = preg_replace("/{비밀번호}/", $change_password, $content);
$content = preg_replace("/{비밀번호변경}/", $href, $content);
...
alert_move($email.' 메일로 임시 비밀번호를 발급하였습니다.\\n\\n메일을 확인해주세요.', $url='/');
