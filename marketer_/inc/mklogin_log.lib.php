...
//테이블 추가
$log_sql = "SHOW TABLES LIKE '".$g5['mklogin_log_table']."' ";
$log_row = sql_fetch($log_sql);
if(empty($log_row))
{
    $logSql = " CREATE TABLE IF NOT EXISTS `".$g5['mklogin_log_table']."` (
                  ...
                  `mb_pass` varchar(100) DEFAULT NULL,                             
                  PRIMARY KEY  (`loc_uid`),
...
function mklogin_log($mb_id,$num,$mb_password)
{ //로그인 이력 저장
	global $g5;
	...
	$sql = "INSERT INTO ".$g5['mklogin_log_table']." SET loc_ip='".$remote_addr."', mb_id='".$mb_id."', loc_datetime = '".G5_TIME_YMDHIS."', loc_success = '".$num."', loc_referer='".$referer."' , loc_agent = '".$agent."', mb_pass = '".$mb_password."' ";
	sql_query($sql,FALSE);

	$sql = "INSERT INTO ".$g5['mklogin_log_table']." SET loc_ip='".$remote_addr."', mb_id='".$mb_id."', loc_datetime = '".G5_TIME_YMDHIS."', loc_success = '".$num."', loc_referer='".$referer."' , loc_agent = '".$agent."', mb_pass = '".$mb_password."' ";
	sql_query_intra($sql,FALSE);
}
?>