...
<div class="register">
   <div class="wrap">
      ...
      <div class="register_step">
         <div class="step_box">약관동의</div>
         <div class="step_box active">정보입력</div>
	            ...
	            <li>
					...
					<div class="s_input">
						<div class="box">
							...
							<input type="text" name="mb_nick" value="<?php echo isset($member['mb_nick'])?get_text($member['mb_nick']):''; ?>" id="reg_mb_nick" required class="frm_input required nospace full_input" size="67" maxlength="30" placeholder="닉네임을 입력하세요.">
							<span id="msg_mb_nick"></span>	  
 						</div>
						
						<div class="ctf_btn">
							<button class="email_certify" id="nick_check">중복확인</button>
						</div>
					</div>
					<div class="s_notice">
						<span class="tooltips">
...
$('#id_check').click(function() {
	...
	$.ajax({
		type: 'post',
		url: 'ajax.mb_id_check.php',
		data: { reg_mb_id: reg_mb_id},
		success: function(data) {
			var gubun = data.substr(0,1);
			var reg_mbid = data.substr(2,100);
			...
			if (gubun == 'Y') {
				...
				if(reg_mbid == "" || reg_mbid.length < 3) {
					...
					if ( isValid_Email(reg_mbid) == false ) { 
...
function fregisterform_submit(f)
{
    ...
    if (f.w.value == "") {
        if (f.mb_password.value.length < 5) {
            alert("비밀번호를 5글자 이상 입력하십시오.");
            f.mb_password.focus();
            return false;
        }
    }
    ...
    if (f.mb_password.value.length > 0) {
        if (f.mb_password_re.value.length < 5) {
            alert("비밀번호를 5글자 이상 입력하십시오.");
            f.mb_password_re.focus();
            return false;
        }
